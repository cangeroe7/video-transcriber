...
export function VideoUpload(props: {
	...
	const [uploadState, setUploadState] = useState<
		...
		| { state: "SUCCESS"; videoId: string }
	>({ state: "IDLE" });
	const router = useRouter();
	const [videoUploadId, setVideoUploadId] = useState<number | null>(null);
	...
	const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
		...
		setVideoUploadId(videoUpload.data.id);
		const checksum = await computeSHA256(selectedFile);
		setVideoChecksum(checksum);

		try {
			const data = await createVideoMutation.mutateAsync({
				title: videoTitle,
			...
			setUploadState({ state: "SUCCESS", videoId: data.id ?? "" });
		} catch (error) {
			console.error("Database error:", error);
			setUploadState({
				state: "FAILURE",
				failure: "Database upload error",
			});
		} finally {
			setLoading(false);
		}
	...
	useEffect(() => {
		if (uploadState.state === "SUCCESS" && videoChecksum) {
			router.push(`/v/${uploadState.videoId}`);
		}
	}, [uploadState, router]);
		...
		<div>
			<div className="flex-shrink-0">
				<Button
					...
					size="lg"
					className="transform rounded-[999px] bg-black px-8 py-3 text-lg font-semibold text-white shadow-lg transition-all duration-200 hover:scale-105 hover:from-green-700 hover:to-emerald-700 hover:shadow-xl"
				>
					Start now
				</Button>
			</div>
						...
						<form onSubmit={handleSubmit}>
							...
							>
								{videoPreview ? (
									...
									<p>
										Drag and drop a video here, or click to
										select
									</p>
								)}
								<input
